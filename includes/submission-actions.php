...
function badgeos_process_submission_review( $post_id ) {
	...
	// and our nonce is valid
	// and the user is allowed to edit the post
	// and we've gained a denied status
	if (
		!empty( $_POST )
		&& isset( $_POST['post_type'] )
		&& ( 'submission' == $_POST['post_type'] || 'nomination' == $_POST['post_type'] ) //verify post type is submission or nomination
		...
		&& wp_verify_nonce( $_POST['wp_meta_box_nonce'], 'init.php' ) //verify nonce for security
		&& current_user_can( 'edit_post', $post_id ) //check if current user has permission to edit this submission/nomination
		&& ( 'denied' == $_POST['_badgeos_submission_status'] || 'denied' == $_POST['_badgeos_nomination_status']  ) //verify user is denieing a submission or nomination
		&& get_post_meta( $post_id, '_badgeos_nomination_status', true ) != 'denied' //if nomination is already denied, skip it
		&& get_post_meta( $post_id, '_badgeos_submission_status', true ) != 'denied' //if submission is already denied, skip it
	) {
		...
		// Revoke the achievement from the user
		if ( $achievement_id && $user_id ) {

			badgeos_revoke_achievement_from_user( absint( $achievement_id ), absint( $user_id ) );
...
function badgeos_save_submission_data() {
	...
	// ensure we don't save the same form twice
...
function badgeos_save_comment_data( $post_id = 0 ) {
	...
	if ( ! isset( $_POST['badgeos_comment_submit'] ) || ! isset( $_POST['badgeos_comment'] ) || ! isset( $_POST['badgeos_post_id']) )
		return;
//	echo '<pre>SAVE COMMENT DATA, POST ID = '. $post_id .'</pre>';
		
	// only process comment data for correct posting
	if($_POST['badgeos_post_id'] != $post_id)
		return;	
...
//	echo '<pre>SAVE COMMENT DATA,  = '. print_r($comment_data,true) .'</pre>';
...
function badgeos_get_comments_for_submission( $post_id = 0 ) {
	...
	if ( !$comments )
		return $comment_data;
//	echo '<pre>'. htmlentities( print_r( $comments, true ) ) .'</pre>';
